...
RCT_EXPORT_METHOD(sendMessage: (int) accountId destination: (NSString *) destination message:(NSString*) text callback:(RCTResponseSenderBlock) callback) {
    @try {
        PjSipEndpoint* endpoint = [PjSipEndpoint instance];
        PjSipAccount *account = [endpoint findAccount:accountId];
        [endpoint sendMessage:account destination:destination message:text];

        callback(@[@TRUE, destination, text]);
    }
    @catch (NSException * e) {
        callback(@[@FALSE, e.reason]);
    }
